            ...
            <div class="search_bd">
                ...
                <dl class="filter_item" id="provice_select">
                    ...
                    <dd>
                        ...
                        <a href="javascript:" v-for="(item,index) in provinces" :data-key="item.key"
                           :class="province==item.key?'provinceItem select':'provinceItem'"
                           @click="selectProvince(item.key,item.value)">
                            {{item.value}}
                        </a>
                ...
                <dl class="filter_item" id="city_select" v-show="cityClick&&cityList">
                    ...
                    <dd>
                        ...
                        <a href="javascript:" v-for="(value,key) in cityList" :data-key="key"
                           @click="selectCity(key,value)" :class="city==key?'select':''">
                            {{value}}
                        </a>
                    </dd>
                </dl>
                ...
                <dl class="filter_item" id="district_select" v-show="districts">
                    ...
                    <dd>
                        ...
                        <a href="javascript:" v-for="(value,key) in districts" :data-key="key"
                           @click="selectDistrict(key,value)" :class="district==key?'select':''">
                            {{value}}
                        </a>
                    </dd>
                </dl>
                ...
                <dl class="filter_item" id="favorite_select" v-show="moreShow">
                    <dt>收藏：</dt>
                    <dd>
                        <a href="javascript:" :class="{'select':favoriteType==false}"
                           @click="selectFavorite(false)">不限</a>
                ...
                <div class="moreBtn" @click="moreShow=!moreShow">
                    <em class="bor1"></em>
                    <em class="bor2"></em>
                    <span>{{moreShow?'收起':'更多筛选'}}<i
                            :class="moreShow?'el-icon-arrow-up':'el-icon-arrow-down'"></i></span>
                </div>
            </div>

            <div id="selected" class="bar" style="display: block;"
                 v-show="provinceName||qualityType!=0||JSON.stringify(wasteCode) !== '{}'">
                <div class="barct">
                    ...
                    <span id="wasteCodeCondition" v-show="JSON.stringify(wasteCode) !== '{}'">
								<em>含：(</em>
								<span>
									<span class="par" v-for="(value,key) in wasteCode"
                                          v-if="typeof value=='number'||typeof value=='string'">
										<em>{{key}}</em>
										<a href="javascript:" @click="selectWaste(wasteInfoList[value],value)"><i
                                                class="el-icon-close"></i></a>
									</span>
									<span v-else>
										<span class="par" v-for="(item,index) in value" v-if="item.select">
											<em>{{item.code}}</em>
											<a href="javascript:" @click="clearEightCode(item)"><i
                                                    class="el-icon-close"></i></a>
										</span>
									</span>
								</span>
                ...
                buy: function (item, index) {
                    if (!item.isPay) {
                        this.payCallback = function () {
                            ...
                            var src = appPath + '/entRelease/cfBuy.htm?ticketId=' + getParam('ticketId') + '&releaseId=' + releaseId + "&breadcrumbName=资源池" + facilitatorEntIdStr;
                            window.location = src;
                        };
