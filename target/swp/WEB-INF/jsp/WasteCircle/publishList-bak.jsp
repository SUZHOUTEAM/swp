                    ...
                    <div v-if="inquiryList.length>0" style="margin: -9px 10px 0;">
                        <div class="buy_item panel panel-body" v-for="item in inquiryList">
                            ...
                            <div style="margin-top: 9px;text-align: right" v-if="item.inquiryType!=='REFERENCE_PRICE'">
                                <span class="status" :class="statusClass[item.busiStatus]"  v-show="item.busiStatus=='待确认'" >{{item.busiStatus}}</span></el-col>
                                <el-button type="text" class="comment_btn" @click="fold(item,index)" v-if="item.evaluated"><i class="comment_icon commented"></i>{{item.commentShow?'收起评价':'已评价，查看'}}<i class="el-input__icon el-icon-arrow-down" :class="{'is-reverse':item.commentShow}"></i></el-button>
                                <el-button type="text" class="comment_btn" @click="fold(item,index)" v-else><i class="comment_icon"></i>评价<i class="el-input__icon el-icon-arrow-down" :class="{'is-reverse':item.commentShow}"></i></el-button>
                            ...
                            <div class="comment_container" v-show="item.commentShow&&item.evaluated">
                                <p>查看评价<el-button style="float: right" @click="addComment(item,index)">增加评论</el-button></p>
