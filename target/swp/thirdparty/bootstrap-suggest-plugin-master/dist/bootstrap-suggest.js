    ...
    */
    ...
    function refreshDropMenu($input, data, options) {
        ...
        if (options.showHeader) {
            html.push('<thead><tr>');
            if (options.tdSortFields.length > 0) {// zhugl 增加列表列的显示顺序控制
                var indexSort = 0;
                for (indexSort = 0; indexSort < options.tdSortFields.length; indexSort ++) {
                    if (inEffectiveFields(options.tdSortFields[indexSort], options) === false) {
                        continue;
                    }
                    
                    if (index === 0 ) {
                        //表头第一列记录总数
                        html.push('<th>' + (options.effectiveFieldsAlias[options.tdSortFields[indexSort]]) + '(' + len + ')' + '</th>');
                    } else {
                        html.push('<th>' + (options.effectiveFieldsAlias[options.tdSortFields[indexSort]]) + '</th>');
                    }
                    index++;
                }
            } else {
        ...
        for (i = 0; i < len; i++) {
            ...
            if (options.tdSortFields.length > 0) {// zhugl 增加列表列的显示顺序控制
                for (indexSort = 0; indexSort < options.tdSortFields.length; indexSort ++) {
                    ...
                    if(options.tdSortFields[indexSort]=='addenterwastename' || options.tdSortFields[indexSort]=='addenterwastedesc'){
                    	var value = data.value[i][options.tdSortFields[indexSort]];
                    	var title = value.replace("<font color='red'>","").replace("</font>","");
                    	var changedTitle = title.replace("<font color='red'>","").replace("</font>","");
                    	if(changedTitle!=''&&changedTitle.length >20 ){
                    		tds.push('<td data-name="', options.tdSortFields[indexSort],'" ','title="',changedTitle, '">',changedTitle.substring(0,20)+"..." , '</td>');
                    	}else if (changedTitle!=''&&changedTitle.length <=20){
                    		tds.push('<td data-name="', options.tdSortFields[indexSort], '">',value , '</td>');
                    	}else{
                    		tds.push('<td  data-name="', options.tdSortFields[indexSort], '">', value, '</td>');
                    	}
                        ...
                        tds.push('<td data-name="', options.tdSortFields[indexSort], '">', data.value[i][options.tdSortFields[indexSort]], '</td>');
                    }
    ...
    function ajax(options, keyword) {
        ...
        var ajaxParam = {
            ...
            url: function() {
               ...
               // return options.url + type + keyword; zhugl 修改 防止ie下url参数乱码 2016/08/11
                return encodeURI(options.url + type + keyword);
            }(),
            dataType: 'json',
            timeout: 5000
        };
            ...
            return self.each(function() {
                ...
                if (! options.showBtn) {
                    $input.css('border-radius', '4px')
                      //  .parents('.input-group:eq(0)').css('width', '100%') zhugl修改
