...
<%--<link rel="stylesheet" href="<%=appPath%>/main/pc/css/style_new.css">--%>
<link rel="stylesheet" href="<%=appPath%>/main/pc/css/money-dialog.css?1">
<link rel="stylesheet" href="<%=appPath%>/css/wastecircle/flagship.css?2">
<section style="margin-bottom: 0 !important;padding-bottom: 50px">
	<div id="app" v-cloak>
			<div class="banner">
				<div class="banner-content">
					<el-row>
						<el-col :span="18" class="banner-title">易废网{{provinceObj[cantonCode]}}旗舰店
							<a href="javascript:;" class="chat-btn"  @click="contact()" title="联系TA">
								联系TA</a>
						</el-col>
						<el-col :span="6" align="right">
							<a href="javascript:;" @click="goInquiry" v-if="inquiryShow" class="banner-btn">询价></a>
						</el-col>
					</el-row>
			...
			<div class="panel panel-body" id="meal">
...
<script src="<%=appPath%>/main/common/provinces.js?2"></script>
...
<script>
    var vue=new Vue({
        ...
        data: {
            ...
            remainTime:5,
            money:0,
            inquiryShow:localStorage.entType=='PRODUCTION',
            cantonCode:localStorage.province,
            provinceObj:provinceObj,
            buyMeal:getParam('buyMeal')
        },
        created:function() {
        },
        mounted:function () {
        ...
        methods:{
            scrollToMeal: function () {
                if(this.buyMeal){
                    this.$nextTick(function () {
                        setTimeout(function () {
                            $('html,body').animate({scrollTop:$('#meal').offset().top}, 1000);
//                            document.getElementById('meal').scrollIntoView(true);
                        },10)
                    });
				}
            },
...
//                        bindServiceId:this.currentReleaseId,
			...
			contact:function () {
                collectingUserBehavior('<%=ticketId%>','FLAGSHIP_CONTACT_HIM','联系了:'+facilitatorEnt.entName);
                getEnterpriseContacts(facilitatorEnt.entId,facilitatorEnt.entName,'旗舰店');
            },
            goInquiry:function () {
                collectingUserBehavior('<%=ticketId%>','FLAGSHIP_INQUIRY');
